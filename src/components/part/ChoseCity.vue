<template>
  <div class="chooseCity">
    <el-select v-model="model1" placestyle="min-width:150pxmax-width: 160px" @on-change="deleteModel(1)">
      <el-option v-for="item in firstFloorDatas" :value="item.id" :key="item.id">{{ item.name }}</el-option>
    </el-select>
    <!-- <el-select
      v-model="model2"
      style="min-width:150pxmax-width: 160px"
      v-if="showSecond"
      @on-change="deleteModel(2)"
    >
      <el-option v-for="item in secondFloorDatas" :value="item.id" :key="item.id">{{ item.name }}</el-option>
    </el-select> -->
    <el-select v-model="model3" style="min-width:150pxmax-width: 160px" v-if="showThird">
      <el-option v-for="item in thirdFloorDatas" :value="item.id" :key="item.id">{{ item.name }}</el-option>
    </el-select>
<!--  -->
...
<script>
var cityDatas = require('@/js/cityDatas')
export default {
  name: 'chooseCity',
  data () {
    return {
